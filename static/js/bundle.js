...
// https://gist.github.com/patriciogonzalezvivo/670c22f3966e662d2f83
  ...
  );
  {
    ...
    palette = ["#03045e", "#0077b6", "#00b4d8", "#90e0ef", "#90e0ef"];
    palette = [
      "#012a4a",
      "#013a63",
      "#01497c",
    ...
    palette = pls[pls.length - 1];
    const hexToRGBAUint8 = (colors) => new Uint8Array(
      colors.flatMap((hex) => [
        ...hex.slice(1).match(/.{2}/g).map((v) => parseInt(v, 16)),
        255
      ])
    );
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColor: { value: new Color(11644321) }
      }
    });
  ...
  window.onload = function() {
    ...
    function updateEye(mx, my) {
      ...
      let outer = 200;
      function getoff(inner, outer2, vec2) {
        let d = dist(vec2.x, vec2.y);
        if (d > outer2) {
          vec2.x *= outer2 / d;
          vec2.y *= outer2 / d;
          d = dist(vec2.x, vec2.y);
        }
        let ratio = d / outer2;
        return {
          x: ratio * vec2.x * inner / outer2,
          y: ratio * vec2.y * inner / outer2
        };
      }
    ...
    function update_guy(mx, my) {
      ...
      let guyy = bounds_guy.y + 1e4;
      let vec = { x: mx - guyx, y: my - guyy };
      let angle2 = Math.tanh(vec.x / vec.y);
      let ratio = Math.min(Math.abs(vec.x), bounds_guy.width / 2) / bounds_guy.width / 2;
      angle2 *= ratio;
      angle2 = Math.abs(angle2) > 0.08 ? 0.08 * Math.sign(angle2) : angle2;
      body.style.transform = "skew(" + angle2 + "rad) translateX(" + 20 * ratio * Math.sign(vec.x) + "px)";
      blush.style.transform = "skew(" + angle2 + "rad) translateX(" + 20 * ratio * Math.sign(vec.x) + "px)";
    }
    document.onresize = (e) => {
      update_bounds();
      update_guy(0, 0);
    ...
    document.getElementById("guy").onclick = (e) => {
      ...
      if (x - 20 > 1 / 3 * bound.width && x - 20 < 2 / 3 * bound.width && y > 1 / 3 * bound.height) {
    ...
    function blink() {
      ...
      blink_timer = setTimeout(blink, 250 + Math.random() * 3e3);
    }
